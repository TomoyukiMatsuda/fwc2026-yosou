...
// ============================================================================
// FIFA 2026 W杯 ベスト32 3位チーム公式配置表（Annex C / 全495通り）
//
// 出典: FIFA World Cup 2026 Regulations, ANNEXE C
//   "COMBINATIONS FOR EIGHT BEST THIRD-PLACED TEAMS"
//   https://digitalhub.fifa.com/m/636f5c9c6f29771f/original/FWC2026_regulations_EN.pdf
//   取得日 2026-06-08 / Wikipedia "Template:2026 FIFA World Cup third-place table"
//   と495行を完全照合済み（不一致0・候補制約違反0・同組対戦0）。
//
// どの8グループの3位が通過するかで、各3位スロット試合(74,77,79,80,81,82,85,87)へ
// 入るグループが公式に一意確定する。候補制約を満たすだけの素朴な割当では公式と
// 異なる配置になりうる（495通り中487通りでズレる）ため、この公式表を直接引く。
//
// データは公式PDFから機械生成。手編集禁止（再生成して差し替えること）。
// ============================================================================

/** キー = 3位通過8グループをアルファベット昇順で連結（例 "EFGHIJKL"） */
type ThirdKey = string;

/** 3位通過の定員（公式 Annex C が対象とする組合せサイズ） */
const THIRD_PLACE_COUNT = 8;

/** 3位スロット試合ID(74,77,79,80,81,82,85,87) → そのスロットに入るグループの3位 */
const OFFICIAL_TABLE: Record<ThirdKey, Record<number, GroupId>> = {
...
/** 3位通過グループ集合 → ルックアップキー（重複除去・昇順連結） */
export function thirdPlacementKey(groups: GroupId[]): ThirdKey {
  return [...new Set(groups)].sort().join("");
}

/**
 * 公式 Annex C による3位配置を引く。
 * ちょうど8グループ確定時のみ有効。未確定(件数≠8)や未知の組合せは null を返す
 * （呼び出し側がフォールバックする）。
 * @returns 試合ID → グループ（その組の3位がそのスロットに入る）／ null
 */
export function lookupOfficialThirdPlacement(
  groups: GroupId[],
): Record<number, GroupId> | null {
  if (new Set(groups).size !== THIRD_PLACE_COUNT) return null;
  return OFFICIAL_TABLE[thirdPlacementKey(groups)] ?? null;
}

/** 公式表のエントリ数（= C(12,8) = 495）。健全性確認・テスト用にexport。 */
export const OFFICIAL_TABLE_SIZE = Object.keys(OFFICIAL_TABLE).length;
